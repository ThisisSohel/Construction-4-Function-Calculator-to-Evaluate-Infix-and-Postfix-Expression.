...
typedef long long int ll;    // LL is the suffix for long long, which is 64-bit on most C/C++ implementations.
...
void postFixOpp(string str1)
{
    ...
    for(int i=0; i<l; i++)
    {
        ...
        else if(Digit(str1[i]))
        {
            ...
            {
                opp=(opp*10)+(str1[i]-'0');  //str[i] - '0' converts the character at position i to a numeric digit.
                i++;
            }
            i--;
            Stack.push(opp);
...
void inFix()
{
    ...
    cin.ignore();       //clear buffer before taking new input
    getline(cin,str);   //taking input in str
